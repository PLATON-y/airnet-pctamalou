...
# 🔹 Beacon Combiné (INVITE + BEACON)
async def send_beacon():
    while True:
        # Payload combiné : INVITE en clair + BEACON chiffré
        invite_payload = f"INVITE:{NODE_ID}".encode()
        beacon_payload = f"BEACON:{NODE_ID}:{time.time()}".encode()
        encrypted_beacon = pctamalou_encrypt(beacon_payload) + public_key.public_bytes(
            serialization.Encoding.PEM, serialization.PublicFormat.SubjectPublicKeyInfo)
        combined_payload = invite_payload + b"||" + encrypted_beacon  # Séparateur clair
